...
def run(model_id, setting, ESMv='3B', tensor_board=False):
    ...
    for k in range(5):
        ...
        model_full_name = (model_id + config.mode) % (k+1)
        model_path = config.save_path % model_full_name
        
        if tensor_board:
            if not os.path.isdir('./tb'):
                os.mkdir('./tb')
            writer = SummaryWriter(log_dir=f'./tb/{model_full_name}', comment='_bce_from_scratch', filename_suffix=model_full_name, flush_secs=1)
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.5, patience=2, verbose=1, 
                                                           mode='min', threshold=0.001, threshold_mode='abs', cooldown=1)    
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tensor_board', dest='tensor_board', action='store_true', default=False, help='Use tensorbaord to monitor the training process')
    ...
    model_id = f'PAI_3Bv{args.version}'
    
    print(f'============ {model_id} Training ============')
    setting = cfg.Model_SetUp(name=model_name, device=args.device)
    setting.get_setting().dropout = dropout

    run(model_id=model_id, setting=setting, ESMv=args.ESMv, tensor_board=args.tensor_board)
