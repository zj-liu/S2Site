...
class myPDBList(Bio.PDB.PDBList):
    ...
    http://www.pdb.org/.
    """
    def __init__(self, structures_folder, *args, **kwargs):
        self.structure_folder = structures_folder
        kwargs['pdb'] = self.structure_folder
        super().__init__(*args,**kwargs)
        self.alphafold_server = 'https://alphafold.ebi.ac.uk/' # entry/Q13469
        # self.pdb_server = 'ftp://ftp.ebi.ac.uk/pub/databases/pdb/' # UK
        #self.pdb_server = 'ftp://ftp.pdbj.org' # JP 
        #self.pdb_server = 'https://s3.rcsb.org'
    ...
    def retrieve_pdb_file(self, code, obsolete=False, pdir=None, file_format=None, overwrite=False):
        ...
        if not overwrite:
            if os.path.exists(final_file):
                if self._verbose:
                    print("Structure exists: '%s' " % final_file, end='\r')
                return final_file

        # Retrieve the file
        if self._verbose:
            print("Downloading PDB structure '%s'..." % code, end='\r')
        try:
            # urlcleanup()
...
def parse_str(str):
    ...
    if len(str_split) == 1 or is_PDB_identifier(str.split('/')[-1]) or is_UniProt_identifier(str.split('/')[-1]):
...
def getPDB(identifier_string, structures_folder, biounit=True, 
           verbose=True, download_noFound=False):
    ...
    if not (is_pdb | is_uniprot):
        ...
        if len(location.split('/')) == 1:
            l1 = structures_folder + location + '.pdb'
            l2 = structures_folder + location + '.cif'
            if os.path.exists(l1):
                location = l1
            elif os.path.exists(l2):
                location = l2
        elif len(location.split('/')) > 1:
            lc = ['.pdb','.cif', '.bioent', '.ent', '_bioentry.cif']
            for l in lc:
                l1 = location + l
                if os.path.exists(l1):
                    location = l1
        ...
        if os.path.exists(location1):
        ...
        elif download_noFound:
            pdb_downloader = myPDBList(verbose=verbose, structures_folder=structures_folder)
            if biounit:
            ...
            raise FileNotFoundError(f'PDB is not found in both {location1}, {location2}, {location3}')
...
def load_chains(pdb_id=None,
                         ...
                         structures_folder='',
                         dockground_indexing=False, biounit=True,verbose=True, download_noFound=False):
    ...
    if (file is None) & (pdb_id is not None):
        file = getPDB(pdb_id, biounit=biounit, structures_folder=structures_folder, download_noFound=download_noFound)[0]
    else:
